Asana Module Development Template Specification
...
* Actual Duration: 17-18 weeks from kickoff to "Ready for Launch" (not 16 weeks as initially documented) → planned breaks like Christmas holidays
* Full Cycle: 26 weeks (178 days) including 10-week buffer to "Go Live"
* Week 1 Special Treatment: 10 days for storyboarding (vs 5 days for weeks 2-8) (it's a 10-day window to get that worked on)
...
Phase	Duration	Days	Weeks	Key Milestone
	...
	Critical Note: Clarifications for the 10-week buffer between "Ready for Launch" and "Go Live":
* Standard for all modules → not a standard, just depends when the program the development for that specific module starts. So the module buffer (if it exists) depends on the program development lifecycle.
* If a buffer exists, it tends to get used, which we would prefer to avoid.
* Academic year alignment requirement → the development is always done against launch dates.
* Client-specific customization → sometimes clients do have a preference for that, but Andrew would prefer to negotiate around that. When a buffer exists, Andrew would require the management team to line-manage the academic resources.
...
4. Launch Section Separate: "Go live" gets its own section to emphasize the 10-week buffer[a]
________________
...
Field Name	Type	Options/Format	Purpose
	...
	Go Live Date[b]	Date	DD/MM/YYYY	Actual launch to students
...
4.4 Section: Launch
Duration: 1 day for milestone + [XX] days buffer
...
Task: Week 1 - Storyboarding Duration: 10 days Start: Launch Date - 156 days Finish: Launch Date - 147 days Assignees: Learning Designer + SME
Subtasks:
...
Task: Week [N] - Storyboarding Duration: 5 days Assignees: Learning Designer + SME
Subtasks:
...
Task: Week [N] - Build Duration: 5 days Assignee: Learning Technologist Depends on: Week [N] - Storyboard Final draft agreed Start: 1 week after corresponding storyboard starts
...
Task: Week 1 and 2 review Duration: 5 days Start: Launch Date - 130 days (11/03/2025 in example) Assignee: Academic Reviewer Depends on: Week 2 - Build Section: Week 1 (for organizational purposes)
Description:
...
Task: Weeks 3 to 8 review Duration: 5 days Start: Launch Date - 20 days (12/22/2025 in example - Christmas week!) Assignee: Academic Reviewer Depends on: Week 8 - Build Section: Finalization
Description:
...
Task: Film shoot - first batch Duration: 6 days Start: Launch Date - 116 days (11/10/2025 in example) Finish: Launch Date - 111 days (11/15/2025) Assignee: Digital Learning Team Depends on: Week 4 - Build (runs in parallel with Week 5 development) Section: Finalization (for organizational purposes, though timing overlaps with Week 5)
Custom Fields:
...
Task: Film shoot - second batch Duration: 5 days Start: Launch Date - 58 days (12/08/2025 in example) Finish: Launch Date - 54 days (12/12/2025) Assignee: Digital Learning Team Depends on: Week 8 - Build Section: Finalization
Description:
...
Task	LD	LT	SME
	...
	Final draft agreed	✓		✓
	Build		✓
	Peak Concurrent: LD + SME (during editing) Sequential: LT starts after LD/SME complete
...
Trigger: All subtasks marked complete Action: Change task custom field "QA Status" to "In Review"
...
Trigger: Section "Initiation & Planning" complete Action: Update project custom field "Module Status" to "In Development"
Trigger: Section "Finalization" complete Action: Update project custom field "Module Status" to "Ready"
Trigger: Task "Go live" marked complete Action: Update project custom field "Module Status" to "Launched"
9.2 Notification Rules
Rule: Notify Next Resource
Trigger: Task "Week N - Storyboard Final draft agreed" marked complete Action:
...
Trigger: Custom field "Blocker Status" changed to anything except "None" Action:
...
Trigger: New section "Week N" created from template Action: Automatically link dependencies:
...
Trigger: Task moved with incomplete dependencies Action:
...
Trigger: Task name contains "Storyboard Initial LD Draft" Action: Auto-assign to Learning Designer from project custom field
Trigger: Task name contains "Build" Action: Auto-assign to Learning Technologist from project custom field
Trigger: Task name contains "review" (lowercase) Action: Auto-assign to Academic Reviewer from project custom field
Rule: Capacity Warning
Trigger: Person assigned to >3 tasks with same due date Action:
...
Trigger: Task marked complete with "QA Status" = "Changes Requested" Action:
* Reopen task
* Add comment: "Cannot complete - QA changes requested. Please address feedback first."
* Notify task assignee
Rule: Academic Review Rejection Workflow
Trigger: Task "Week 1 and 2 review" custom field "QA Status" → "Changes Requested" Action:
...
Trigger: Project custom field "Launch Date" modified Action:
...
Trigger: Task becomes overdue Action:
...
11. Questions for Andrew
Before finalizing template implementation, we must clarify the following with Andrew:
...
| 40 | Film shoot - first batch | Finalization | Film shoot - first batch | (task) | 6d | | Digital Learning Team | | 66 | Film shoot - second batch | Finalization | Film shoot - second batch | (task) | 5d | | Digital Learning Team | | 67 | Proofreading | Finalization | Proofreading | (task) | 5d | | Editorial Team | | 68 | Weeks 3 to 8 review | Finalization | Weeks 3 to 8 review | (task) | 5d | | Academic Reviewer | | 69-70 | Corrections | Finalization | Corrections | (task) | 5d | Learning Designer/Technologist | SME | | 71 | Ready for launch | Finalization | Ready for launch | (task) | 1d | | | | 72 | Go live | Launch | Go live | (task) | -- | | |
________________
...
Status: DRAFT - Requires Andrew's review and clarification of questions in Section 11 before implementation Next Action: Schedule review meeting to address Section 11 questions Target Implementation Date: TBD after clarifications Tuesday Meeting Demo: To be prepared after template finalized


[a]NOTE: Launch is not Andrew's task. Andrew's job is to get everything ready for launch. Launch is the client's responsibility.
[b]Launch date and go live date are the same thing. we have 'ready to launch' and 'launch'
[c]Has been super inconsistent with current client, tends to be lighter touch than proofreading.